...
def get_xsound_list():
    sounds = requests.get(
      'https://lebkuchen-fm.fly.dev/api/x-sounds',
      headers = {
        'Authorization': 'Basic a0Vjr3P9gr0Ekc9A9w1C2a3Xzl4ydlwA',
      }
    ).json()['sounds']

    ss = '\n'.join([
      'setXSoundList({',
      *list(map(lambda s: '  { name = "' + s['name'] + '" },', sounds)),
      '})',
    ])

    with open("./server/go.lua", "w") as f:
      f.write(ss)
      f.close()

def exec_fm_command(command):
  body = {'text': command}
  requests.post(
    'https://lebkuchen-fm.fly.dev/api/commands/text',
    json = body,
    headers = {
      'Authorization': 'Basic a0Vjr3P9gr0Ekc9A9w1C2a3Xzl4ydlwA',
    })
...
with tempfile.NamedTemporaryFile(prefix='main', suffix='.lua') as luafile, tempfile.TemporaryDirectory(suffix='.pdx') as pdxdir:
  ...
  with open('./server/go.lua', 'rb') as infile:
